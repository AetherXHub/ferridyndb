...
fn main() {
    ...
    let db = if Path::new(&cli.database).exists() {
        FerridynDB::open(&cli.database)
    } else {
        FerridynDB::create(&cli.database)
    };
    let db = match db {
        Ok(db) => db,
        Err(e) => {
...
fn run_exec_mode(db: &FerridynDB, commands: &[String], json_mode: bool) -> i32 {
...
fn run_pipe_mode(db: &FerridynDB, json_mode: bool) -> i32 {
    let mode = if json_mode {
        OutputMode::Json
    } else {
        OutputMode::Pretty
