...
fn exec_query(
    ...
    let result = rt.block_on(client.query(table, pk, cond, limit, fwd, None))?;
...
fn exec_scan(
    ...
    let result = rt.block_on(client.scan(table, limit, None))?;
    Ok(CommandResult::QueryResult(result))
}
...
fn exec_query_index(
    ...
    let fwd = if desc { Some(false) } else { None };
    let result = rt.block_on(client.query_index(table, index_name, key_value, limit, fwd))?;
    Ok(CommandResult::QueryResult(result))
}
