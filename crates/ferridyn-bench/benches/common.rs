...
use ferridyn_core::api::batch::SyncMode;
...
enum FerridynBenchOp {
    Insert { key: [u8; KEY_SIZE], value: Vec<u8> },
    Remove { key: [u8; KEY_SIZE] },
}

pub struct FerridynBenchDatabase {
    db: FerridynDB,
    count: Arc<AtomicU64>,
}

impl FerridynBenchDatabase {
...
impl BenchDatabase for FerridynBenchDatabase {
    type C<'db>
        = FerridynBenchConnection
    where
        Self: 'db;
...
pub struct FerridynBenchConnection {
    db: FerridynDB,
    count: Arc<AtomicU64>,
}

impl BenchDatabaseConnection for FerridynBenchConnection {
    type W<'db>
        = FerridynBenchWriteTransaction
    where
        Self: 'db;
    type R<'db>
        = FerridynBenchReadTransaction
    where
        Self: 'db;

    fn set_sync(&mut self, sync: bool) -> bool {
        if sync {
            self.db.set_sync_mode(SyncMode::Full);
        } else {
...
pub struct FerridynBenchWriteTransaction {
    db: FerridynDB,
    ops: Vec<FerridynBenchOp>,
    count: Arc<AtomicU64>,
}
...
impl BenchWriteTransaction for FerridynBenchWriteTransaction {
    type W<'txn>
        = FerridynBenchInserter<'txn>
    where
        Self: 'txn;

    fn get_inserter(&mut self) -> Self::W<'_> {
        FerridynBenchInserter { txn: self }
    }
...
pub struct FerridynBenchInserter<'a> {
    txn: &'a mut FerridynBenchWriteTransaction,
}

impl BenchInserter for FerridynBenchInserter<'_> {
    fn insert(&mut self, key: &[u8], value: &[u8]) -> Result<(), ()> {
        let mut k = [0u8; KEY_SIZE];
        k.copy_from_slice(key);
        self.txn.ops.push(FerridynBenchOp::Insert {
            key: k,
            value: value.to_vec(),
        });
...
pub struct FerridynBenchReadTransaction {
    db: FerridynDB,
    count: Arc<AtomicU64>,
}

impl BenchReadTransaction for FerridynBenchReadTransaction {
    type T<'txn>
        = FerridynBenchReader
    where
        Self: 'txn;
...
pub struct FerridynBenchReader {
    db: FerridynDB,
    count: Arc<AtomicU64>,
}

impl BenchReader for FerridynBenchReader {
    type Output<'out>
        = Vec<u8>
    where
        Self: 'out;
    type Iterator<'out>
        = FerridynBenchIterator
    where
        Self: 'out;
...
pub struct FerridynBenchIterator {
    items: std::vec::IntoIter<(Vec<u8>, Vec<u8>)>,
}

impl BenchIterator for FerridynBenchIterator {
    type Output<'out>
        = Vec<u8>
    where
        Self: 'out;
