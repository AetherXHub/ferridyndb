...
/// Shared page cache type alias (lock-free concurrent cache).
pub type PageCache = Arc<moka::sync::Cache<PageId, [u8; PAGE_SIZE]>>;
...
/// A read-only [`PageStore`] backed by a file descriptor with a lock-free
/// concurrent page cache (moka).
///
/// Uses `pread` (`FileExt::read_exact_at`) so it is safe to share the
/// underlying file handle across threads. Cache reads are lock-free;
/// cache inserts use fine-grained internal sharding with no global lock.
...
impl BufferedPageStore {
    ...
    /// Create a new buffered store with a shared page cache.
...
impl PageStore for BufferedPageStore {
    fn read_page(&self, page_id: PageId) -> Result<Page, StorageError> {
        ...
        // Check cache if available (lock-free).
        if let Some(cache) = &self.cache
            && let Some(buf) = cache.get(&page_id)
        {
            return Ok(Page::from_bytes(buf, page_id));
        }
