...
struct DatabaseInner {
    ...
    /// LRU page cache shared across all read stores and write transactions.
...
impl FerridynDB {
    ...
    fn commit_txn(&self, txn: Transaction, state: &mut DatabaseState) -> Result<(), Error> {
        ...
        // Write all overlay pages to disk and update the page cache.
        {
            let mut cache = self.inner.page_cache.lock();
            for (&page_id, data) in store.overlay() {
