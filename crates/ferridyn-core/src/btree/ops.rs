...
use super::PageStore;
use super::node::{
    BTREE_DATA_OFFSET, internal_cell_child, internal_cell_key, internal_find_child,
    internal_rightmost_child, internal_search_slot, internal_set_rightmost_child, leaf_cell_key,
    leaf_cell_value, leaf_find_exact, leaf_next_leaf, leaf_search_slot, leaf_set_next_leaf,
    make_internal_cell, make_leaf_cell,
};
...
pub fn search(
    ...
    let page = store.read_page(root_page)?;
    match page.page_type()? {
        PageType::BTreeInternal => {
            let child_id = internal_find_child(&page, key);
            search(store, child_id, key)
        }
        PageType::BTreeLeaf => {
...
fn split_leaf(
    ...
    let mut all_cells: Vec<Vec<u8>> = {
        let sp_ref = SlottedPageRef::new(&old_page, BTREE_DATA_OFFSET);
    ...
    clear_page_slots(&mut old_page);
    {
        let mut sp = SlottedPage::new(&mut old_page, BTREE_DATA_OFFSET);
    ...
    {
        let mut sp = SlottedPage::new(&mut new_page, BTREE_DATA_OFFSET);
        for cell in &all_cells[mid..] {
            sp.insert(cell)?;
        }
    }
...
/// Update the child pointer in an internal cell at the given slot index.
fn update_internal_cell_child(page: &mut Page, slot_index: usize, new_child: PageId) {
    let cell = {
        let sp_ref = SlottedPageRef::new(page, BTREE_DATA_OFFSET);
    ...
    let new_cell = make_internal_cell(&cell_key, new_child);

    {
        let mut sp = SlottedPage::new(page, BTREE_DATA_OFFSET);
...
fn split_internal(
    ...
    clear_page_slots(&mut old_page);
    {
        ...
        for i in 0..mid {
            ...
            let cell = make_internal_cell(&key, children[i]);
            sp.insert(&cell)?;
        }
    }
...
/// while preserving the page type, page ID, and the 8 bytes at [32..40].
fn clear_page_slots(page: &mut Page) {
    ...
    for byte in &mut data[BTREE_DATA_OFFSET..] {
        *byte = 0;
    }
    page.mark_dirty();
}
...
mod tests {
    ...
    fn test_multi_level_split() {
        ...
        // This forces many leaf splits. With ~1000 entries, we get ~250+ leaves,
        // which exceeds the capacity of a single internal node (~193 children),
        // forcing internal splits and creating 3+ tree levels.
