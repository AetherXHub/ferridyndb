...
//!
//! ```text
//! [0..40]   common page header (PageType::BTreeLeaf)
//! [40..48]  next_leaf: u64 LE (PageId of next sibling, 0 if none)
//! [48..]    slotted page region (slot array grows forward, cells grow backward)
//! ```
//!
...
//!
//! ```text
//! [0..40]   common page header (PageType::BTreeInternal)
//! [40..48]  rightmost_child: u64 LE (child for keys >= all stored keys)
//! [48..64]  rightmost_child_checksum: u128 LE (Merkle checksum of rightmost child page)
//! [64..]    slotted page region
//! ```
//!
//! Each internal cell: `[key_len: u16 LE][key bytes][child_page_id: u64 LE][child_checksum: u128 LE]`
//!
//! The child in each cell is the pointer for keys LESS THAN that cell's key.
//! The rightmost child handles keys >= the last key.
//! Each child pointer is accompanied by the expected XXH3_128 checksum of that child page,
//! forming part of the Merkle checksum tree.
...
/// Byte offset where leaf node slotted data begins (after next_leaf field).
pub const LEAF_DATA_OFFSET: usize = 48;

/// Byte offset where internal node slotted data begins
/// (after rightmost_child + rightmost_child_checksum fields).
pub const INTERNAL_DATA_OFFSET: usize = 64;
...
pub fn internal_set_rightmost_child(page: &mut Page, child: PageId) {
    let data = page.data_mut();
    data[40..48].copy_from_slice(&child.to_le_bytes());
}

/// Read the Merkle checksum of the rightmost child from an internal page.
pub fn internal_rightmost_child_checksum(page: &Page) -> u128 {
    let data = page.data();
    u128::from_le_bytes(data[48..64].try_into().unwrap())
}

/// Set the Merkle checksum of the rightmost child on an internal page.
pub fn internal_set_rightmost_child_checksum(page: &mut Page, checksum: u128) {
    let data = page.data_mut();
    data[48..64].copy_from_slice(&checksum.to_le_bytes());
}

/// Extract the key portion from an internal cell.
///
/// Cell format: `[key_len: u16 LE][key bytes][child_page_id: u64 LE][child_checksum: u128 LE]`
...
/// Extract the Merkle checksum of the child from an internal cell.
pub fn internal_cell_child_checksum(cell: &[u8]) -> u128 {
    let key_len = u16::from_le_bytes([cell[0], cell[1]]) as usize;
    let offset = 2 + key_len + 8;
    u128::from_le_bytes(cell[offset..offset + 16].try_into().unwrap())
}

/// Build an internal cell from a key, child pointer, and child Merkle checksum.
pub fn make_internal_cell(key: &[u8], child: PageId, child_checksum: u128) -> Vec<u8> {
    let key_len = key.len() as u16;
    let mut cell = Vec::with_capacity(2 + key.len() + 8 + 16);
    cell.extend_from_slice(&key_len.to_le_bytes());
    cell.extend_from_slice(key);
    cell.extend_from_slice(&child.to_le_bytes());
    cell.extend_from_slice(&child_checksum.to_le_bytes());
    cell
}

/// Find which child to follow for `key` in an internal page.
///
/// Returns `(child_page_id, expected_child_checksum)` for Merkle verification.
///
...
pub fn internal_find_child(page: &Page, key: &[u8]) -> (PageId, u128) {
...
mod tests {
    ...
    #[test]
    fn test_internal_rightmost_child_checksum() {
        let mut page = Page::new(1, PageType::BTreeInternal);
        assert_eq!(internal_rightmost_child_checksum(&page), 0);
        let cs: u128 = 0xDEAD_BEEF_CAFE_BABE_1234_5678_9ABC_DEF0;
        internal_set_rightmost_child_checksum(&mut page, cs);
        assert_eq!(internal_rightmost_child_checksum(&page), cs);
    }

    #[test]
    fn test_internal_cell_roundtrip() {
        let cs: u128 = 0x1234_5678_9ABC_DEF0_DEAD_BEEF_CAFE_BABE;
        let cell = make_internal_cell(b"key", 42, cs);
        assert_eq!(internal_cell_key(&cell), b"key");
        assert_eq!(internal_cell_child(&cell), 42);
        assert_eq!(internal_cell_child_checksum(&cell), cs);
    }
    ...
    fn test_internal_find_child_single_key() {
        ...
        internal_set_rightmost_child_checksum(&mut page, 200);
        {
            let mut sp = SlottedPage::new(&mut page, INTERNAL_DATA_OFFSET);
            // Key "M" with left child 10, checksum 100
            sp.insert(&make_internal_cell(b"M", 10, 100)).unwrap();
        }

        // Key < "M" -> child 10, checksum 100
        assert_eq!(internal_find_child(&page, b"A"), (10, 100));
        // Key == "M" -> rightmost child (20, 200)
        assert_eq!(internal_find_child(&page, b"M"), (20, 200));
        // Key > "M" -> rightmost child (20, 200)
        assert_eq!(internal_find_child(&page, b"Z"), (20, 200));
    }
    ...
    fn test_internal_find_child_multiple_keys() {
        ...
        internal_set_rightmost_child_checksum(&mut page, 300);
        {
            let mut sp = SlottedPage::new(&mut page, INTERNAL_DATA_OFFSET);
            sp.insert(&make_internal_cell(b"D", 10, 100)).unwrap();
            sp.insert(&make_internal_cell(b"H", 20, 200)).unwrap();
        }

        // key < "D" -> child 10, checksum 100
        assert_eq!(internal_find_child(&page, b"A"), (10, 100));
        // key == "D" -> child 20, checksum 200 (>= D, < H)
        assert_eq!(internal_find_child(&page, b"D"), (20, 200));
        // key between D and H -> child 20, checksum 200
        assert_eq!(internal_find_child(&page, b"F"), (20, 200));
        // key == "H" -> rightmost (30, 300)
        assert_eq!(internal_find_child(&page, b"H"), (30, 300));
        // key > "H" -> rightmost (30, 300)
        assert_eq!(internal_find_child(&page, b"Z"), (30, 300));
    }
